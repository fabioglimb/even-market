...
export class EvenHubBridge {
    ...
    try {
      const result = await this.bridge.createStartUpPageContainer(
        new CreateStartUpPageContainer({
          containerTotalNum: 1,
          textObject: [makeText(TEXT_FULL, 'EvenMarket\nLoading...', 1)],
        }),
      );
  ...
  /** Chart layout: text top (event capture) + 3 images = 4. */
  ...
  async switchToTextLayout(text: string): Promise<boolean> {
    ...
    try {
      await this.bridge.rebuildPageContainer(
        new RebuildPageContainer({
          containerTotalNum: 1,
          textObject: [makeText(TEXT_FULL, text, 1)],
        }),
      );
      this._currentLayout = 'text';
      return true;
  ...
  /** Update text (container id 1 in both layouts). */
  ...
  onEvent(handler: (event: EvenHubEvent) => void): void {
    ...
    this.unsubEvents = this.bridge.onEvenHubEvent((event: EvenHubEvent) => { handler(event); });
  }

  dispose(): void { this.unsubEvents?.(); this.unsubEvents = null; this.bridge = null; }
}
